...
[post](https://svaditya.github.io/blog/MRS_Data_Analysis.html). The the
earlier post we performed data cleaning and feature engineering on the
data set. We also studies the relationship between the label (gender in
this case) and other covariates. In this post we will start of from
where we left and play around with various machine learning functions
from the powerful RevoScaleR package. Just to refresh lets quickly take
...
There are 8 variables in total. Please note that we will treating the
...
Clearly this is case of a two-class classification. We will building 3
different models to evaluate the performance and choose the best
performing model. Before we get started lets split our data in to train
and test data sets using the `rxSplit` function.
...
Lets start with a stepwise logistic regression. This helps us to
automatically choose the best variable to be included in the model.
...
Lets build a decision forest with the training data.
